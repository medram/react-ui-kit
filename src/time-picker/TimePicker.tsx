...
function TimePickerInput({
  ...
  // flag and prevIntKey are control values for the keypress handler — they are
  // never rendered, so we store them in refs to avoid re-rendering the input on
  // every keypress.
  const flagRef = React.useRef<boolean>(false)
  const prevIntKeyRef = React.useRef<string>("0")
  const flagTimerRef = React.useRef<ReturnType<typeof setTimeout> | null>(null)

  // Allow the user to enter the second digit within 2 seconds. Scheduled from
  // the handler that flips flag=true so we don't need an effect to react.
  const setFlag = React.useCallback((value: boolean) => {
    flagRef.current = value
    if (flagTimerRef.current) {
  ...
  // No unmount cleanup needed: the timer's callback only mutates refs
  // (flagRef and flagTimerRef) which are harmless to write after unmount.
  ...
  const calculateNewValue = (key: string) => {
     ...
     */
    if (picker === "12hours") {
      if (flagRef.current && calculatedValue.slice(1, 2) === "1" && prevIntKeyRef.current === "0")
        return `0${key}`
    }

    return !flagRef.current ? `0${key}` : calculatedValue.slice(1, 2) + key
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (["ArrowUp", "ArrowDown"].includes(e.key)) {
      ...
      if (flagRef.current) setFlag(false)
      const tempDate = date ? new Date(date) : new Date()
      onDateChange?.(setDateByType(tempDate, newValue, picker, period))
    }
    if (e.key >= "0" && e.key <= "9") {
      if (picker === "12hours") {
        prevIntKeyRef.current = e.key
      }
  ...
  return (
    <div className="flex items-center  gap-2">
      <label htmlFor={id} className="cursor-pointer">
        <Clock className={`mr-2 h-4 w-4 ${disabled && "opacity-50 cursor-not-allowed"} `} />
      </label>
      <TimePickerInput
...
export type { DateTimePickerRef, TimePickerType }
