...
import SelectInput, { SelectOptions } from "@/components/ui/select-input"
...
type BaseChartCardProps<T> = {
  ...
  value?: T
  defaultValue?: T
  onValueChange?: (value: T, option: SelectOptions<T>) => void
...
export function BaseChartCard<T extends number | string>({
  ...
  const [uncontrolledOption, setUncontrolledOption] = useState<SelectOptions<T> | null>(() => {
    if (!options?.length) return null
    return options.find((option) => option.value === defaultValue) ?? options[0]
  })
  const selectedOption =
    value === undefined ? uncontrolledOption : (options?.find((option) => option.value === value) ?? null)
        ...
        {options && options.length > 1 && (
          <SelectInput
            ...
            onChange={(nextValue, nextOption) => {
              ...
              if (value === undefined) setUncontrolledOption(nextOption)
              onValueChange?.(nextValue as T, nextOption)
            }}
          />
        )}
